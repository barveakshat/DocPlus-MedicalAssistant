...
const Prescriptions: React.FC = () => {
  ...
  const fetchTodayLogs = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .eq('patient_user_id', effectiveUserId)
            ...
            {loading ? (
              ...
              active.map((rx) => (
                <PrescriptionCard
                  ...
                  takenToday={todayLogs.has(rx.id)}
                  onLogDose={handleLogDose}
                  loggingDose={loggingDose}
                />
              ))
            )}
          </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium text-slate-700 mb-1 block">Drug Name</label>
                <Input value={drugName} onChange={(e) => setDrugName(e.target.value)} placeholder="e.g. Amoxicillin" />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-700 mb-1 block">Dose</label>
            ...
            <div>
              <label className="text-sm font-medium text-slate-700 mb-1 block">Instructions (optional)</label>
              <Textarea
                value={instructions}
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="font-bold text-slate-900">{rx.drug_name}</span>
            <span className="text-slate-500 text-sm">· {rx.dose}</span>
            <Badge className={`text-[10px] border ${STATUS_COLORS[rx.status] ?? ''}`}>{rx.status}</Badge>
          ...
          {rx.patient_name && (
            <p className="text-xs text-slate-500 mb-1">Patient: {rx.patient_name}</p>
          )}
          <p className="text-sm text-slate-600">{rx.frequency} · {rx.duration}</p>
          {rx.instructions && (
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {userRole === 'patient' && rx.status === 'active' && !takenToday && (
          <Button
            size="sm"
            variant="outline"
            className="h-8 text-xs text-[#5442f5] border-[#5442f5]/30 hover:bg-[#5442f5]/5"
        ...
        {userRole === 'doctor' && rx.status === 'active' && (
          <>
            <Button
              ...
              onClick={() => onUpdateStatus(rx.id, 'completed')}
            ...
            <Button
              ...
              onClick={() => onUpdateStatus(rx.id, 'discontinued')}
              title="Discontinue"
            >
